...
Physical data set | EN v4.2.1 (Good et al 2013)
Physical variables| Salinity averaged over 250-600m depth on a pixel-by-pixel basis
Physical forecast method |  Persistence from the most recent data product 
...
* 20180301.v03.0 Updated EN4 data set to 4.2.1. Added forecast expressed as an anomaly. Added "forecast basis" summary table. Added supplementary information comparing with survey track and current oceanographic state.
...
#Setup salinity data
sal.b <- raster(sal.src)
sal.df <- data.frame(coordinates(sal.b),
                     value=sal.b[])
sal.df$val.rounded <- factor(sprintf("%2.1f",round(sal.df$value*5)/5))
...
sal.b.hires <- disaggregate(sal.b,
                            fact=res(sal.b)/res(forecast.r),
                            method="bilinear")
sal.hires.df <- data.frame(coordinates(sal.b.hires),
                          sal=sal.b.hires[])
...
ggplot(sal.df,aes(x,y))+
  geom_raster(aes(fill=val.rounded))+
  coord_quickmap(xlim=plot.xlims,ylim=plot.ylims,expand=FALSE)+
  geom_contour(data=bath.df,aes(z=bath,linetype=factor(..level..)),
               breaks=c(1000,2000),col="black")+
  geom_contour(data=sal.hires.df,aes(z=sal),
               breaks=sal.isohalines,size=1,col="blue")+
  borders(xlim=plot.xlims,ylim=plot.ylims,
          fill="black",col="black")+
  scale_fill_brewer(palette="YlOrRd",direction = 1)+
  theme_bw()+  
  labs(x="",y="",fill="Salinity",linetype="Depth (m)")

```
*Figure S1. Observed salinity distribution for January 2018. The isohalines defining the suitable salinity window (35.3 - 35.5) at spawning depth for Blue Whiting are marked with a blue contour line. The 500m and 2000m isobaths are added for reference.*
